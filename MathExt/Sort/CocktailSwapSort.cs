...
namespace dotnet.common.MathExt
{
    public partial class SortAlgorithm
    {
        ...
        public static void CocktailSort(IList<DataItem> dataItems)
        {
            for (int i = 0; i < dataItems.Count; i++)
            {
                if (!AdjustBackwardThenForward(dataItems, i))
                {
                    return;//没有发生交换，则已经有序。
                }
        ...
        /// <returns></returns>
        private static bool AdjustBackwardThenForward(IList<DataItem> dataItems, int traversalIndex)
        {
            bool hasSwap = false;
            ...
            startIndexOfSortedBackward--;
            for (int forwardIndex = startIndexOfSortedBackward; forwardIndex > endIndexOfSortedForward + 1; forwardIndex--)
            {
